var UserController = function(userModel) {
    ...
    var readUserByEmail = function(email, callback) {
        userModel.findOne({ email: email }, function(err, user) {
            ...
            if (!user) {
                ...
                return callback(err, new ApiResponse({ success: true, extras: { userProfileModels: user }}));
            }
        });
    };
    ...
    var userIsValid = function(email, password, callback) {
        userModel.findOne({
            ...
            if (!user) callback(err, new ApiResponse({
                success: false,
                extras: {
                    msg: ApiMessages.EMAIL_NOT_FOUND
                }
            })); // User not found case
